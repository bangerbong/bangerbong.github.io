document.addEventListener('DOMContentLoaded', () => {
    ...
    const desktopIcons = Array.from(document.querySelectorAll('#desktop-icons-container .icon')); // Get icons specifically
    const stickyNote = document.getElementById('sticky-note');
    const desktopFileItems = document.querySelectorAll('.desktop-file-item');
    const draggableDesktopElements = document.querySelectorAll('.icon, #sticky-note, .desktop-file-item'); // All draggable things
    ...
    if (bootScreen) {
        setTimeout(() => {
            ...
            bootScreen.style.pointerEvents = 'none'; // Allow interaction with page below
            // Remove from DOM after transition
            setTimeout(() => {
                if(bootScreen.parentNode) bootScreen.parentNode.removeChild(bootScreen);
            }, 1000); // Match CSS transition duration
        }, 3500); // Duration of boot screen visibility (includes animation)
    }


    // --- Initial Desktop Element Positioning via JavaScript ---
    function layoutDesktopElements() {
        if (window.innerWidth >= 769) { // Only apply this grid on desktop
            // Define your desired grid positions for icons
            const iconPositions = [ 
                { el: desktopIcons.find(i => i.dataset.window === 'aboutWindow'), top: 60, left: 60 },
                { el: desktopIcons.find(i => i.dataset.window === 'resumeWindow'), top: 60, left: 170 },
                { el: desktopIcons.find(i => i.dataset.window === 'computerWindow'), top: 60, left: 280 },
                { el: desktopIcons.find(i => i.dataset.window === 'contactWindow'), top: 60, left: 390 },
                { el: desktopIcons.find(i => i.dataset.window === 'posterWindow'), top: 170, left: 60 },
                { el: desktopIcons.find(i => i.dataset.window === 'photoWindow'), top: 170, left: 170 },
                { el: desktopIcons.find(i => i.dataset.window === 'paintWindow'), top: 170, left: 280 },
                { el: desktopIcons.find(i => i.dataset.window === 'minesweeperWindow'), top: 170, left: 390 },
                { el: desktopIcons.find(i => i.dataset.window === 'logoBrandingWindow'), top: 280, left: 60 },
                { el: desktopIcons.find(i => i.dataset.window === 'videoWindow'), top: 280, left: 170 },
                { el: desktopIcons.find(i => i.dataset.window === 'interactiveDesignWindow'), top: 280, left: 280 },
                { el: desktopIcons.find(i => i.dataset.window === 'recycleBinWindow'), top: 280, left: 390 },
                { el: ieIcon, top: 400, left: 60 } // Position IE icon specifically
            ];
            ...
            iconPositions.forEach(pos => {
                if (pos.el) {
                    ...
                    pos.el.classList.add('positioned');
                }
            });
            
            // Position sticky note for desktop
            if(stickyNote) {
                stickyNote.style.top = '60px';
                stickyNote.style.left = `calc(100% - 260px - 60px)`; // width and right offset
                stickyNote.style.transform = 'rotate(1.5deg)';
                stickyNote.classList.add('positioned');
            }

            // Position desktop file items
            const fileitem1 = document.getElementById('fileitem1');
            const fileitem2 = document.getElementById('fileitem2');
            if (fileitem1) { fileitem1.style.top = '380px'; fileitem1.style.left = '480px'; fileitem1.classList.add('positioned'); }
            if (fileitem2) { fileitem2.style.top = '450px'; fileitem2.style.left = '180px'; fileitem2.style.transform = 'rotate(-5deg)'; fileitem2.classList.add('positioned'); }

        } else {
            // On smaller screens, CSS media queries will handle flex layout for icons.
            // Ensure elements are visible if they were hidden by JS initially.
            desktopIcons.forEach(icon => {
                icon.style.position = ''; icon.style.top = ''; icon.style.left = ''; // Clear JS positions
                icon.classList.add('positioned');
            });
            if(stickyNote) {
                stickyNote.style.position = ''; stickyNote.style.top = ''; stickyNote.style.left = ''; // Clear JS positions
                stickyNote.classList.add('positioned');
            }
            desktopFileItems.forEach(item => {
                item.style.position = ''; item.style.top = ''; item.style.left = ''; // Clear JS positions
    ...
    // --- Start Menu Logic --- (same as before)
    ...
    // --- IE Icon Link --- (same as before)
    if (ieIcon) { ieIcon.addEventListener('dblclick', () => { window.open('https://www.linkedin.com/', '_blank'); }); } // Replace with your actual LinkedIn

    // --- Window Management Functions (open, close, active, taskbar) --- (same as before)
    ...
    // --- Initial Page Setup ---
    layoutDesktopElements(); // Position icons, sticky note, file items on load
    
    // Optionally pre-open "About Me" window on desktop
    if (window.innerWidth >= 769) {
        // No need to set makeActive to false if it's the only one, setActiveWindow handles it
        openWindowById('aboutWindow'); 
    }
    window.addEventListener('resize', layoutDesktopElements); // Re-layout if window resizes 
