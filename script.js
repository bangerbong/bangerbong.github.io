document.addEventListener('DOMContentLoaded', () => {
    ...
    const draggableDesktopElements = document.querySelectorAll('.icon, .draggable-desktop-element, .desktop-file-item');
    const desktopIconsContainer = document.getElementById('desktop-icons-container'); // To append icons if needed

    // Start Menu elements
    const startButton = document.getElementById('start-button');
    const startMenu = document.getElementById('start-menu');
    const ieIcon = document.getElementById('ieIcon'); // IE Icon
    const shutdownOverlay = document.getElementById('shutdown-overlay');
    const restartButton = document.getElementById('restart-button');


    let highestZ = 100; 
    let highestDesktopZ = 20; // For icons, sticky note, desktop files
    const openWindowsMap = new Map();

    // --- Initial Desktop Icon Positioning (using inline styles from HTML now) ---
    // JS might still adjust or verify, especially if icons are dynamically added later
    // For now, we trust the HTML inline styles for initial desktop positions.
    // The JS drag logic will correctly use these initial positions.

    // --- Start Menu Logic ---
    if (startButton && startMenu) {
        startButton.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent click from closing menu if menu is already open
            startMenu.classList.toggle('open');
            startButton.classList.toggle('active'); // Visual feedback for start button
        });

        // Close Start Menu if clicking outside
        document.addEventListener('click', (e) => {
            if (startMenu.classList.contains('open') && !startButton.contains(e.target) && !startMenu.contains(e.target)) {
        ...
        startMenu.querySelectorAll('li[data-window]').forEach(item => {
            item.addEventListener('click', () => {
                openWindowById(item.dataset.window);
                startMenu.classList.remove('open');
                startButton.classList.remove('active');
            });
        });
        ...
        const shutdownButtonStartMenu = document.getElementById('shutdown-button-startmenu');
        if(shutdownButtonStartMenu) {
            shutdownButtonStartMenu.addEventListener('click', () => {
                if(shutdownOverlay) shutdownOverlay.style.display = 'block';
                startMenu.classList.remove('open');
                startButton.classList.remove('active');
            });
        }
    }
    if(restartButton && shutdownOverlay) {
        restartButton.addEventListener('click', () => {
            shutdownOverlay.style.display = 'none';
            // Optionally: close all windows or refresh page
            // location.reload(); // To simulate a full restart
        });
    }
    ...
    function openWindowById(winId, makeActive = true) {
        ...
        if (!win) { console.error("Window not found:", winId); return; }
        ...
        if(makeActive) setActiveWindow(win);
        else { win.style.zIndex = highestZ -1; }
    ...
    function createTaskbarItem(winId, title) {
        if (openWindowsMap.has(winId)) { 
            const win = document.getElementById(winId);
            if (win && parseInt(win.style.zIndex) >= highestZ -1 ) { // Check if it's an active or recently opened window
        ...
        if (document.getElementById(winId)?.style.zIndex == highestZ) {
            updateActiveTaskbarItem(winId);
        }
    }
    ...
    draggableDesktopElements.forEach(element => {
        ...
        dragHandle.addEventListener('mousedown', (e) => {
            ...
            // If the element isn't already 'absolute' (e.g. it's in flex flow on mobile from CSS)
            // it will use its inline style 'top' and 'left' from HTML if on desktop.
            // If it was in flex flow, its position must be set to absolute now for dragging.
            if (computedStyle.position !== 'absolute') {
                const rect = element.getBoundingClientRect();
                element.style.left = `${rect.left + window.scrollX}px`;
                element.style.top = `${rect.top + window.scrollY}px`;
                element.style.position = 'absolute'; // THIS IS CRUCIAL
            }
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            if (!hasMovedSignificantly && (Math.abs(e.clientX - (element.offsetLeft + offsetX)) > 5 || Math.abs(e.clientY - (element.offsetTop + offsetY)) > 5)) {
        ...
        // For actual icons that open windows
        if (element.classList.contains('icon') && element.dataset.window) {
    ...
    // Pre-open "About Me" window but don't make it "active" visually initially
    if (window.innerWidth >= 769) { 
        openWindowById('aboutWindow', false); 
        setTimeout(() => {
            const aboutWin = document.getElementById('aboutWindow');
            if (aboutWin && aboutWin.style.display === 'block') {
                let currentlyActive = false;
                document.querySelectorAll('.window .title-bar.active-window-title').forEach(() => currentlyActive = true);
                if (!currentlyActive) { setActiveWindow(aboutWin); }
            }
        }, 350); // Increased delay slightly
    }
    // No need to call layoutDesktopIcons() on resize if relying on media queries for non-desktop primarily.
    // The initial inline styles handle desktop.
});
