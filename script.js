document.addEventListener('DOMContentLoaded', () => {
    ...
    // IE Icon will now be treated as a regular icon that opens 'internetWindow'
    ...
    if (bootScreen) {
        setTimeout(() => {
            ...
            }, 1000); // Should match CSS transition duration for opacity
        }, 3000); // Total time boot screen is visible (includes its own animation + fade out)
    }

    function layoutDesktopElements() {
        const isDesktop = window.innerWidth >= 769;

        // Define desired positions for desktop
        const desktopElementPositions = [
            ...
            { selector: '.icon[data-window="internetWindow"]', top: 400, left: 60 }, // IE icon now opens a window
            // Sticky Note
        ...
        draggableDesktopElements.forEach(el => {
            if (isDesktop) {
                // Find if this element has a predefined desktop position
                ...
                if (posData) {
                    ...
                    // Fallback if not in array (should not happen for icons)
                    // For dynamically added elements later, this might be needed
                     if (!el.style.top || !el.style.left) { // Only if not already dragged
                        el.style.position = 'absolute';
                        el.style.top = '50px'; // Default fallback
                        el.style.left = '50px'; // Default fallback
                     }
                }
            } else { // Mobile/Tablet: clear JS positioning, let CSS flexbox take over
                el.style.position = '';
                el.style.top = '';
                el.style.left = '';
                el.style.transform = ''; // Clear transform for mobile flow
            }
            el.classList.add('positioned'); // Make it visible
        });
    }
    ...
    if (window.innerWidth >= 769 && !openWindowsMap.size) { // Only open About if no other window was opened (e.g. from hash)
        openWindowById('aboutWindow', true); // Make it active now
    }
