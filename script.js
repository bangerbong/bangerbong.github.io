document.addEventListener('DOMContentLoaded', () => {
    ...
    const ieIcon = document.getElementById('ieIcon'); // IE Icon is now a regular icon opening a window
    ...
    function layoutDesktopElements() {
        ...
        if (isDesktop) {
            const iconPositions = [ 
                ...
                { selector: '#ieIcon', top: 400, left: 60 } // #ieIcon is unique
            ];

            iconPositions.forEach(pos => {
                const iconElement = document.querySelector(pos.selector);
                if (iconElement) {
                    iconElement.style.position = 'absolute'; 
                    iconElement.style.top = `${pos.top}px`;
                    iconElement.style.left = `${pos.left}px`;
                    iconElement.classList.add('positioned');
                }
            });
            ...
            if(stickyNote) {
                ...
                stickyNote.style.left = `calc(100% - 260px - 60px)`;
            ...
            if (fileitem1) { fileitem1.style.position = 'absolute'; fileitem1.style.top = '380px'; fileitem1.style.left = '480px'; fileitem1.classList.add('positioned'); }
            if (fileitem2) { fileitem2.style.position = 'absolute'; fileitem2.style.top = '450px'; fileitem2.style.left = '180px'; fileitem2.style.transform = 'rotate(-5deg)'; fileitem2.classList.add('positioned'); }

        } else { // Mobile/Tablet
            desktopIcons.forEach(icon => {
                icon.style.position = ''; icon.style.top = ''; icon.style.left = '';
                icon.classList.add('positioned');
            });
            if(stickyNote) {
                stickyNote.style.position = ''; stickyNote.style.top = ''; stickyNote.style.left = ''; stickyNote.style.transform = '';
                stickyNote.classList.add('positioned');
            }
             desktopFileItems.forEach(item => {
                item.style.position = ''; item.style.top = ''; item.style.left = ''; item.style.transform = '';
    ...
    // IE Icon now opens a window "internetWindow"
    if (ieIcon) { 
        // It's already handled by data-window="internetWindow" in HTML if it should open a window.
        // No specific JS needed here unless it's a direct link.
        // The dblclick to open its window is handled by the generic icon logic below.
    }
    ...
    if (window.innerWidth >= 769) {
        ...
        setTimeout(() => {
            ...
            if (aboutWin && aboutWin.style.display === 'block') { 
                ...
                if (!currentlyActive || openWindowsMap.size === 1) { // Also make active if it's the only window
